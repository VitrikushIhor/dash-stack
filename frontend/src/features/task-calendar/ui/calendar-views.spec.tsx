...
vi.mock('./month-view/day-cell', () => ({
  DayCell: ({ cell, tasks }: { cell: ICalendarCell; tasks: Task[] }) => (
...
vi.mock('./agenda-view/agenda-day-group', () => ({
  AgendaDayGroup: ({ date, tasks }: { date: Date; tasks: Task[] }) => (
...
describe('Calendar Views & Header Integration', () => {
  ...
  describe('CalendarMonthView', () => {
    it('renders weekdays and grid cell items correctly', () => {
      ...
      render(
        ...
        >
          <CalendarMonthView singleDayTasks={mockEvents} />
        </CalendarProvider>
      )
  ...
  describe('CalendarAgendaView', () => {
    it('renders agenda groups of tasks for current month', () => {
      const selectedDate = new Date('2026-06-10T12:00:00Z')
      render(
        <CalendarProvider
          users={mockUsers}
          ...
          selectedDate={selectedDate}
        >
          <CalendarAgendaView singleDayTasks={mockEvents} />
        </CalendarProvider>
      )
    ...
    it('renders empty placeholder when there are no tasks', () => {
      const selectedDate = new Date('2026-06-10T12:00:00Z')
      render(
        <CalendarProvider
          users={mockUsers}
          ...
          selectedDate={selectedDate}
        >
          <CalendarAgendaView singleDayTasks={[]} />
        </CalendarProvider>
      )

      expect(
        screen.getByText('No tasks scheduled for the selected month')
      ).toBeInTheDocument()
    })
  })
  ...
  describe('CalendarHeader', () => {
    it('displays navigators and buttons, and allows switching views', async () => {
      ...
      render(
        <CalendarProvider
          ...
          view='month'
        >
          <CalendarHeader tasks={mockEvents} />
        </CalendarProvider>
      )
    ...
    it('triggers openCreate modal store action when clicking Add Task button', async () => {
      const user = userEvent.setup()
      const selectedDate = new Date('2026-06-10T12:00:00Z')
      ...
      const addEventBtn = screen.getByRole('button', { name: /add task/i })
