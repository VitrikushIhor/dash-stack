...
import { useSubmitMatchScore } from './use-submit-match-score'
...
describe('useSubmitMatchScore', () => {
  ...
  it('does not submit a Match score or request authentication for a confirmed guest', async () => {
    const getMe = vi.spyOn(userApi, 'getMe')
    const { result } = renderSubmission(null)
    await act(async () => {
      await result.current.submitMatchScore('deck-1', 14_500)
      await result.current.submitMatchScore('deck-1', 14_500)
    })
  ...
  it('submits a Match score immediately using the cached user', async () => {
    const getMe = vi.spyOn(userApi, 'getMe')
    const { result } = renderSubmission(mockUser)
    await act(async () => {
      const submission = result.current.submitMatchScore('deck-1', 14_500)
