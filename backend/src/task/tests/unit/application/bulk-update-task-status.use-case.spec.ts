...
describe('BulkUpdateTaskStatusUseCase', () => {
  ...
  it('should throw BadRequestException if status is undefined', async () => {
    await expect(useCase.execute('org-1', ['task-1', 'task-2'], undefined)).rejects.toThrow(
  ...
  it('should bulk update task status to COMPLETED and set completedAt date', async () => {
    ...
    const result = await useCase.execute('org-1', ['task-1', 'task-2'], TaskStatus.COMPLETED);

    expect(result).toEqual({ count: 2 });
    expect(taskRepository.updateMany).toHaveBeenCalledWith('org-1', ['task-1', 'task-2'], {
  ...
  it('should bulk update task status to UPCOMING and clear completedAt date', async () => {
    ...
    const result = await useCase.execute('org-1', ['task-1', 'task-2'], TaskStatus.UPCOMING);
