...
const mockMember = (overrides: Partial<OrganizationMember> = {}): OrganizationMember => ({
  user: {
    id: 'user-1',
...
describe('FindOrganizationMemberUseCase', () => {
  ...
  it('returns member when found', async () => {
    ...
    expect(result).toBe(expected);
    expect(repository.findOrganizationMember).toHaveBeenCalledWith('org-1', 'user-1');
  });

  it('throws MemberNotFoundException when member not found', async () => {
    repository.findOrganizationMember.mockResolvedValue(null);

    await expect(useCase.execute('org-1', 'user-999')).rejects.toThrow(MemberNotFoundException);
  });

  it('exception message contains both orgId and userId', async () => {
    repository.findOrganizationMember.mockResolvedValue(null);

    await expect(useCase.execute('org-1', 'user-999')).rejects.toThrow(/user-999/);
  });
});
