...
describe('UpdateOrganizationUseCase', () => {
  ...
  it('calls repository.update with correct orgId and command', async () => {
    repository.update.mockResolvedValue(mockOrganizationReadModel({ name: 'Updated' }));
  ...
  it('throws with correct message when not found', async () => {
    ...
    await expect(useCase.execute('org-999', { name: 'Ghost' })).rejects.toThrow('org-999');
  });

  it('passes partial update (only description)', async () => {
    repository.update.mockResolvedValue(mockOrganizationReadModel({ description: 'New desc' }));
